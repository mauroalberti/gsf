...
def interpolate_rkf(geoarray: GeoArray, delta_time: Number, pt: Point) -> Tuple[Optional[Point], Optional[float]]:
    """
    ...
    :rtype: tuple of oprional point and optional float.

    Examples:
    """

    k1_vx, k1_vy = point_velocity(geoarray, pt)

    if k1_vx is None or k1_vy is None:
        return None, None

    k2_pt = Point(
        pt.x + (0.25) * delta_time * k1_vx,
        pt.y + (0.25) * delta_time * k1_vy
    )
    ...
    k3_pt = Point(
        pt.x + (3.0 / 32.0) * delta_time * k1_vx + (9.0 / 32.0) * delta_time * k2_vx,
        pt.y + (3.0 / 32.0) * delta_time * k1_vy + (9.0 / 32.0) * delta_time * k2_vy
    )
    ...
    k4_pt = Point(
        pt.x + (1932.0 / 2197.0) * delta_time * k1_vx - (7200.0 / 2197.0) * delta_time * k2_vx + (7296.0 / 2197.0) * delta_time * k3_vx,
        pt.y + (1932.0 / 2197.0) * delta_time * k1_vy - (7200.0 / 2197.0) * delta_time * k2_vy + (7296.0 / 2197.0) * delta_time * k3_vy)
    ...
    k5_pt = Point(
        pt.x + (439.0 / 216.0) * delta_time * k1_vx - (8.0) * delta_time * k2_vx + (3680.0 / 513.0) * delta_time * k3_vx - (845.0 / 4104.0) * delta_time * k4_vx,
        pt.y + (439.0 / 216.0) * delta_time * k1_vy - (8.0) * delta_time * k2_vy + (3680.0 / 513.0) * delta_time * k3_vy - (845.0 / 4104.0) * delta_time * k4_vy)
    ...
    k6_pt = Point(
        pt.x - (8.0 / 27.0) * delta_time * k1_vx + (2.0) * delta_time * k2_vx - (3544.0 / 2565.0) * delta_time * k3_vx + (1859.0 / 4104.0) * delta_time * k4_vx - (
                          11.0 / 40.0) * delta_time * k5_vx,
        pt.y - (8.0 / 27.0) * delta_time * k1_vy + (2.0) * delta_time * k2_vy - (3544.0 / 2565.0) * delta_time * k3_vy + (1859.0 / 4104.0) * delta_time * k4_vy - (
